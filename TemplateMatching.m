...
I = rgb2gray(imread('Image\mrimain.jpg')); % input image with characters
temp = rgb2gray(imread('Template\mriCroppedf.jpg')); %template to be matched
       ...
       %% extracting the local neighbors of input image
        ...
        %if t > 0.97
            ...
            %% holding corrdinates (i,j) where matching
                ...
                x = i; 
                y = j;
                R(x:x+tRow-1, y:y+tCol-1) = S(x:x+tRow-1, y:y+tCol-1)+0.3;
            %% setting flag
                flag = 1;
            %%
                break;
